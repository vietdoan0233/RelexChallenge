...
- historical architecture, handoff, and challenge documentation may retain “KEEPER” when identifying the existing codename — this file, `CLAUDE.md`, and `docs/HANDOFF_*.md` are not rewritten to remove it,
...
- Name-to-subject assignment is strict on two bases, full name and first name (`backend/app/ingestion/name_resolution.py`). "Ana Duarte" and "Ana" are one participant, so a bare first name is rewritten with the full name -- but only when it is at least three letters, appears as no part of any *other* active participant's name, is not an everyday word (stoplist, or mostly lower case in the archive with e-mail addresses excluded), and is not a reserved label. A first name that fails any test (for example two participants named Nadia) is never guessed: it is left unchanged and reported on the preview with its reason. A manifest-reviewed `FIRST_NAME` alias stays authoritative. Verification matches a bare name token as a whole token (so "ana" is not a leak inside "management"); e-mail addresses and multi-word names keep strict substring matching. Reversal restores every position to the vault's one canonical name, so a rewritten bare first name returns as the full name.
