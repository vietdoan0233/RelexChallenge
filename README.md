...
```bash
python scripts/radar.py --limit 10
```

The application also performs this check automatically when the server starts
while fewer than 5 valid Radar findings are stored. Each refresh targets up to
10 cards, so the application keeps a useful target of 5 without ever storing
more than 10. The work runs in the background, so the UI can open immediately
and will refresh while cards are being prepared. The command above remains
available when you intentionally want to force a rebuild. It sends retrieved
evidence excerpts to the configured organizer reasoning service and stores the findings in the runtime database. Curated outside
