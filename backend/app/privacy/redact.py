...
def sanitize_text(text: str, kind: str, target: Target) -> str:
    ...
    while i < len(lines):
        ...
        if header and _mentions(header.group("value"), patterns, email_patterns):
            out.append(f"{header.group('label')} {labels.REDACTED_SENDER}")
            i += 1
            continue
        ...
        for regex in (_RECIPIENT_HEADER, _ATTENDEES_HEADER):
            ...
            if header and _mentions(header.group("value"), patterns, email_patterns):
                line = _redact_list_header(
                    header, patterns, email_patterns, regex is _ATTENDEES_HEADER
                )
                break
        out.append(_inline(line, patterns, email_patterns))
        i += 1
    ...
    return _inline("\n".join(out), patterns, email_patterns)
...
def _inline(line: str, patterns, email_patterns) -> str:
    for pattern in email_patterns:
        line = pattern.sub(labels.REDACTED_PERSON, line)
    for pattern in patterns:
        line = pattern.sub(labels.REDACTED_PERSON, line)
    return line


def _speaker_marker_line(line: str, patterns) -> str | None:
    """'Name 4 minutes 39 seconds' -> '[REDACTED SPEAKER] 4 minutes 39 seconds'."""
    stripped = line.strip()
    for pattern in patterns:
        match = pattern.match(stripped)
        if match and _TIME_PHRASE.match(stripped[match.end() :]):
            return f"{labels.REDACTED_SPEAKER}{stripped[match.end() :]}"
    return None


def _redact_list_header(header, patterns, email_patterns, drop: bool) -> str:
    """Remove (attendees) or mark (recipients) each entry that names the target."""
    sep = "," if drop else ";"
    entries = split_top_level(header.group("value"), sep)
    if len(entries) <= 1 and not drop:
    ...
    for entry in entries:
        if _mentions(entry, patterns, email_patterns):
            if not drop:
                kept.append(labels.REDACTED_PERSON)
        else:
            kept.append(entry)
    joiner = f"{sep} "
